...
class Blocks {
     ...
     * @returns {string[]}
     */
    static linePalette(startType, endType, blocks) {
        console.log(startType, endType, blocks)

        const startColor = this.get(startType)
        const endColor = this.get(endType)

        console.log(JSON.stringify(startColor), JSON.stringify(endColor))
        ...
        for (let i = 0; i < blocks - 2; i++) {
            ...
            types.push(this.getClosestBlockType(lab.l, lab.a, lab.b))
        }
     ...
     * @returns {string}
     */
    static getClosestBlockType(l, a, b) {
        let lowestDistance = Infinity
        let output

        for (const { name, lab } of blockData) {
            const distance = (lab[0] - l) ** 2 + (lab[1] - a) ** 2 + (lab[2] - b) ** 2
